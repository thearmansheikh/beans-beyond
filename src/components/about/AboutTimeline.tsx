...
const MILESTONES = [
  ...
  { year: "Now",  title: "Still going strong", desc: "Hundreds of guests every week — and we're just getting started." },
];
            ...
            {MILESTONES.map(({ year, title, desc }, i) => (
              ...
              >
                ...
                <div className={`flex-1 ${i % 2 === 0 ? "sm:text-right sm:pr-10" : "sm:text-left sm:pl-10"} pl-10 sm:pl-0 bg-white rounded-2xl p-5 border border-[#EEE6DC] group-hover:border-[#D2691E]/25 group-hover:shadow-md group-hover:-translate-y-0.5 transition-all`}>
                  ...
                  <p className="text-sm text-[#333]/60 leading-relaxed">{desc}</p>
                </div>

                {/* Dot */}
                <motion.div
                  className="absolute left-0 sm:left-1/2 sm:-translate-x-1/2 top-1 w-9 h-9 rounded-full bg-[#D2691E] border-4 border-[#F8F4EF] flex items-center justify-center shadow-md shrink-0"
                  initial={{ scale: 0 }}
                  whileInView={{ scale: 1 }}
                  viewport={{ once: true, margin: "-60px" }}
